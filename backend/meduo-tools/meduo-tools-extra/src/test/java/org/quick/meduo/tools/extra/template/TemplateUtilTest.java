...
import org.quick.meduo.tools.core.lang.Dict;
import org.quick.meduo.tools.extra.template.engine.beetl.BeetlEngine;
import org.quick.meduo.tools.extra.template.engine.enjoy.EnjoyEngine;
import org.quick.meduo.tools.extra.template.engine.freemarker.FreemarkerEngine;
import org.quick.meduo.tools.extra.template.engine.rythm.RythmEngine;
import org.quick.meduo.tools.extra.template.engine.thymeleaf.ThymeleafEngine;
import org.quick.meduo.tools.extra.template.engine.velocity.VelocityEngine;
...
public class TemplateUtilTest {
	...
	public void freemarkerEngineTest() {
		// 字符串模板
		TemplateEngine engine = TemplateUtil.createEngine(
				new TemplateConfig("templates", TemplateConfig.ResourceMode.STRING).setCustomEngine(FreemarkerEngine.class));
		Template template = engine.getTemplate("hello,${name}");
		String result = template.render(Dict.create().set("name", "hutool"));
		Assert.assertEquals("hello,hutool", result);
	...
	public void velocityEngineTest() {
		// 字符串模板
		TemplateEngine engine = TemplateUtil.createEngine(
				new TemplateConfig("templates", TemplateConfig.ResourceMode.STRING).setCustomEngine(VelocityEngine.class));
		...
		//ClassPath模板
		engine = TemplateUtil.createEngine(
				new TemplateConfig("templates", TemplateConfig.ResourceMode.CLASSPATH).setCustomEngine(VelocityEngine.class));
	...
	public void enjoyEngineTest() {
		...
		engine = new EnjoyEngine(
				new TemplateConfig("templates", TemplateConfig.ResourceMode.CLASSPATH).setCustomEngine(EnjoyEngine.class));
	...
	public void thymeleafEngineTest() {
		...
		//ClassPath模板
		engine = TemplateUtil.createEngine(
				new TemplateConfig("templates", TemplateConfig.ResourceMode.CLASSPATH).setCustomEngine(ThymeleafEngine.class));
	...
	public void renderToFileTest() {
		TemplateEngine engine = new BeetlEngine(new TemplateConfig("templates", TemplateConfig.ResourceMode.CLASSPATH));
